<?php
namespace Concrete\Package\EasyNews\Controller\SinglePage\Dashboard\Manage;
use Loader;
use CollectionType;
use Page;
use Helper;
...
class DashboardEasyNewsManageController extends DashboardPageController {
	...
	public function edit($cID) {
		...
		if (in_array($news->getCollectionParentID(), array_keys($sections))) {
			...
			$this->redirect('/dashboard/easy_news/manage');
		}
	}
	...
	public function add() {
		...
		if ($this->isPost()) {
			...
			if (!$this->error->has()) {
				...
				$this->saveData($p);
				$this->redirect('/dashboard/easy_news/manage', 'news_added');
			}
		}
	}
	...
	public function update() {
		...
		if ($this->isPost()) {
			...
			if (!$this->error->has()) {
				...
				$this->saveData($p);
				$this->redirect('/dashboard/easy_news/manage', 'news_updated');
			}
		}
	}
	...
	public function remove() {
		...
		if ($this->isPost()) {
			$p = Page::getByID($this->post('newsID'));
			$p->delete();
			$this->redirect('/dashboard/easy_news/manage', 'news_deleted');
		}
	}	
	...
	private function saveData($p) {
		...
		//$data = array('content' => '<h2>'.$this->post('newsTitle').'</h2>'.$this->post('newsBody')); //TODO: think about adding title here
